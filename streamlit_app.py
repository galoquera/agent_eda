...
# -*- coding: utf-8 -*-
"""
App Streamlit — Agente EDA com LangChain + Gemini (+ LangSmith)
- Upload dinâmico de CSV
- Ferramentas de EDA + tendências temporais
- Memória conversacional (RunnableWithMessageHistory)
- Compatível com Gemini tool calling (sem Optional nos args_schema)
"""

import os
import io
import tempfile
from typing import List  # (evitar Optional nos schemas)
...
# ===== LangSmith (opcional): habilita tracing se houver chave em Secrets =====
...
class AgenteDeAnalise:
    def __init__(self, caminho_arquivo_csv: str, session_id: str = "ui_streamlit"):
        ...
        if not os.path.exists(caminho_arquivo_csv):
            raise FileNotFoundError(f"Erro: arquivo '{caminho_arquivo_csv}' não encontrado.")
        ...
        prompt = ChatPromptTemplate.from_messages(
            [
                ("system",
                 ...
                 "- Para relações entre variáveis: 'plotar_mapa_correlacao', 'plotar_dispersao', 'matriz_dispersao'.\n"
                 "- Para 'tabela cruzada': 'tabela_cruzada'.\n"
                 "- Para frequências: 'frequencias_coluna'.\n"
                 "- Para outliers do dataset: 'resumo_outliers_dataset' (se existir), ou por coluna via IQR/Z-score.\n"
                 "- Se o usuário omitir a coluna (histograma, frequências, moda, outliers), use a última coluna citada.\n"
                 "- Para converter 'Time' em datetime: 'converter_time_para_datetime'.\n"
                 "- Para padrões no tempo: 'tendencias_temporais'.\n"
    ...
    # -------------------------
    # Definição das ferramentas (SEM Optional nos schemas)
    # -------------------------
    def _definir_ferramentas(self):
        ...
        class ModaInput(BaseModel):
            coluna: str = Field(description="Coluna para calcular moda (qualquer tipo).")

        class DispersaoInput(BaseModel):
            x: str = Field(description="Coluna X (numérica).")
            y: str = Field(description="Coluna Y (numérica).")
            hue: str = Field(default="", description="Coluna categórica para colorir (opcional, use '' se não quiser).")

        class PairplotInput(BaseModel):
            colunas: str = Field(default="", description="Lista separada por vírgula (opcional, deixe '' para auto).")
            hue: str = Field(default="", description="Coluna categórica (opcional, use '').")
        ...
        class TimeConvertInput(BaseModel):
            origem: str = Field(default="", description="YYYY-MM-DD HH:MM:SS (opcional, use '' p/ relativo).")
            unidade: str = Field(default="s", description="Unidade de 'Time': s/ms/m/h.")
            nova_coluna: str = Field(default="", description="Nome da nova coluna (opcional).")
        ...
        class TendenciasInput(BaseModel):
            ...
            freq: str = Field(default="D", description="Frequência: H/D/W/M.")
        ...
        return [
            ...
            StructuredTool.from_function(
                ...
                description="Describe numérico das colunas.",
            ),
            StructuredTool.from_function(
    ...
    def matriz_dispersao(self, colunas: str = "", hue: str = "") -> str:
        if colunas:
        ...
        g = sns.pairplot(df_plot, vars=cols, hue=(hue if hue else None), corner=True, diag_kind="hist", plot_kws=dict(s=15, alpha=0.7))
        st.pyplot(g.fig)
    ...
    def tendencias_temporais(self, coluna: str, freq: str = "D") -> str:
        ...
        if not pd.api.types.is_numeric_dtype(self.df[coluna]):
            return f"A coluna '{coluna}' precisa ser numérica."
        # Detecta coluna temporal
        ts_col = None
        if "Time_dt" in self.df.columns:
            ts_col = "Time_dt"
        elif "Time" in self.df.columns:
            # cria Time_dt ancorado no epoch por padrão
        ...
        ax.set_xlabel("Tempo"); ax.set_ylabel(coluna); ax.grid(True, linestyle="--", alpha=0.3)
        ax.legend()
        st.pyplot(fig)
        self.memoria_analises.append(f"Tendência exibida: {coluna}, freq={freq}.")
    ...
    # Pré-processador simples para perguntas
...
st.caption("Envie um CSV e faça perguntas. Use também a seção de **tendências temporais**.")
...
if st.session_state.agente is not None:
    ...
    if num_cols:
        valor_col = st.selectbox(
            "Coluna numérica", options=num_cols,
            index=num_cols.index("Amount") if "Amount" in num_cols else 0
        )
        ...
        with col2:
            ...
            if st.button("Gerar tendência"):
                msg = agente.tendencias_temporais(coluna=valor_col, freq=freq)
                st.success(msg)
    else:
